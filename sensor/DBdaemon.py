...
def repeat():
    ...
    # 插入数据到sensor1表
    cursor.execute(
        """INSERT INTO SENSOR1(tistamp, temp, humi) VALUES (%f,%f,%f)"""
        % (float(tim.timestamp()), float(temperature1), float(humidity1))
    )
    ...
    # 插入数据到sensor2表
    cursor.execute(
        """INSERT INTO SENSOR2(tistamp, als, lux) VALUES (%f,%f,%f)"""
        % (float(tim.timestamp()), float(light_Data1), float(light_Data2))
    )
...
if __name__ == "__main__":
    ...
    ltr390.LTR390UV_SetMeasurement(
        ltr390.LTR390UV_MEASUREMENT_RATE_100MS | ltr390.LTR390UV_RESOLUTION_18BIT
    )
